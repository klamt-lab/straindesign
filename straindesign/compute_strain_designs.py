...
def compute_strain_designs(model: Model, **kwargs):
    ...
    allowed_keys = {
        MODULES, SETUP, SOLVER, MAX_COST, MAX_SOLUTIONS, 'M', 'compress', KOCOST, 
        KICOST, GKOCOST, GKICOST, REGCOST, SOLUTION_APPROACH, 'gene_kos', 'advanced', 
        'use_scenario', T_LIMIT
    }
    if SETUP in kwargs:
    ...
    with redirect_stdout(io.StringIO()), redirect_stderr(
            io.StringIO()), DisableLogger():  # suppress standard output from copying model
    ...
    if kwargs['gene_kos']:
        ...
        g_itv = {
            g for g in list(uncmp_gko_cost.keys()) +
            list(uncmp_gki_cost.keys())
        }
        r_itv = {
            r for r in list(uncmp_ko_cost.keys()) +
            list(uncmp_ki_cost.keys())
        }
    ...
    [
        uncmp_ko_cost.pop(er)
        for er in essential_reacs
        if er in uncmp_ko_cost
    ]
    ...
    if kwargs['gene_kos']:
        if kwargs['compress'] is True or kwargs['compress'] is None:
            ...
            uncmp_gko_cost = remove_irrelevant_genes(
                cmp_model, essential_reacs, uncmp_gki_cost,
                uncmp_gko_cost)
        ...
        reac_map = extend_model_gpr(cmp_model, uncmp_gko_cost,
                                    uncmp_gki_cost)
    ...
    uncmp_ko_cost = extend_model_regulatory(cmp_model,
                                                    uncmp_reg_cost,
                                                    uncmp_ko_cost)
    ...
    [
        cmp_ko_cost.pop(er)
        for er in essential_reacs
        if er in cmp_ko_cost
    ]
    essential_kis = set(cmp_ki_cost[er]
                        for er in essential_reacs
                        if er in cmp_ki_cost)
    # Build MILP
    kwargs1 = kwargs
    ...
    kwargs_milp = {k:v for k,v in kwargs.items() if k in [
        SOLVER, MAX_COST, 'M'
    ]}
    ...
    setup.update({  MODULES: orig_sd_modules, 
                    KOCOST:  orig_ko_cost,
                    KICOST:  orig_ki_cost,
    ...
    if kwargs['gene_kos']:
        setup.update({
                GKOCOST:orig_gko_cost,
                GKICOST:orig_gki_cost
            })
