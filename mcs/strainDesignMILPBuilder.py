...
class StrainDesignMILPBuilder:
    ...
    def __init__(self, model: cobra.Model, mcs_modules: List[mcs_module.MCS_Module], *args, **kwargs):
        ...
        elif self.M == None and self.solver == 'glpk':
        ...
        if self.ko_cost == None:
            self.ko_cost = {rid:1 for rid in reac_ids}
        if self.ki_cost == None:
            self.ki_cost = {}
        ...
        if self.max_cost == None:
            self.b_ineq = [np.inf, np.inf]
        else:
            self.b_ineq = [np.inf,self.max_cost]
        ...
        z_kos_kis = [1 if (not self.z_non_targetable[i]) and (not self.z_inverted[i]) else -1 if self.z_inverted[i] else 0 for i in range(0,self.num_z)]
    ...
    def dualize(self,A_ineq_p, b_ineq_p, A_eq_p, b_eq_p, c_p, lb_p, ub_p, z_map_constr_ineq_p, z_map_constr_eq_p, z_map_vars_p) -> \
        ...
        A_ineq = sparse.vstack((sparse.hstack((np.transpose(-A_eq_p[:,x_geq0]),np.transpose(-A_ineq_p[:,x_geq0]))),\
                                sparse.hstack((np.transpose( A_eq_p[:,x_leq0]),np.transpose( A_ineq_p[:,x_leq0]))))).tocsr()
        ...
        z_map_vars        = sparse.hstack((z_map_constr_eq_p,z_map_constr_ineq_p,sparse.csc_matrix((self.num_z,len(lb_inh_bounds)+len(ub_inh_bounds))))).tocsc()
    ...
    def link_z(self):
        ...
        # (5) Link z-variables with bigM constraints, where available
        # (6) Translate remaining inequalities back to equalities if applicable
        # (7) Add indicator constraints
        # 
        #

        # 2. Split knockable equality constraints into foward and reverse direction
        ...
        self.z_map_constr_eq = self.z_map_constr_eq[:,[False if i in knockable_constr_eq[1] else True for i in range(0,n_rows_eq)]]

        # 3. Translate all variable knockouts to inequality knockouts
        ...
        ub_constr_A = sparse.csr_matrix(([ 1]*len(knockable_vars_geq0),(range(0,len(knockable_vars_geq0)),knockable_vars_geq0)),[len(knockable_vars_geq0),numvars])
        ub_constr_b = [0]*len(knockable_vars_geq0)
        lb_constr_A = sparse.csr_matrix(([-1]*len(knockable_vars_leq0),(range(0,len(knockable_vars_leq0)),knockable_vars_leq0)),[len(knockable_vars_leq0),numvars])
        ...
        # 4. Use LP to identify M-values for knockable constraints
        ...
        if ray.is_initialized():
            ...
            class M_optimizer(object):
                def __init__(self): # The LP object is only constructed once upon Actor creation.
                    self.lp = solver_interface.MILP_LP(A_ineq=M_A_ineq, b_ineq=M_b_ineq, A_eq=M_A_eq, b_eq=M_b_eq, lb=M_lb, ub=M_ub)
        ...
        Ms = [Ms[np.array([i==j for j in knockable_constr_ineq]).nonzero()[0][0]] if i in knockable_constr_ineq else np.nan for i in range(self.A_ineq.shape[0])]
        
        # 5. Link constraints to z-variables for available (and also arbitrary high) Ms
        self.z_map_constr_ineq = self.z_map_constr_ineq.tocsc()
        self.A_ineq            = self.A_ineq.todok()
        # iterate through knockoable constraints
        ...
        # 6. Translate back remaining inequalities to equations if applicable
        ...
        Ab_find_dupl = sparse.hstack((sparse.diags(first_entry_A_ineq_sign)*self.A_ineq, \
                                      sparse.coo_matrix(sparse.diags(first_entry_A_ineq_sign)*self.b_ineq).transpose(), \
        ...
        # 7. Link remaining (in)equalities to z via indicator constraints
        #    and remove them from the static problem
        #    - first ineqs then eqs
        
        ic_binv  = np.append(self.z_map_constr_ineq[:,knockable_constr_ineq_ic].indices,self.z_map_constr_eq[:,knockable_constr_eq_ic].indices)
        ...
        ic_indicval = np.append(self.z_map_constr_ineq[:,knockable_constr_ineq_ic].data,self.z_map_constr_eq[:,knockable_constr_eq_ic].data)
    ...
    def reassign_lb_ub_from_ineq(self,A_ineq, b_ineq, A_eq, b_eq, lb, ub, z_map_constr_ineq, z_map_constr_eq, z_map_vars) -> \
            ...
            z_map_constr_ineq = z_map_constr_ineq[:,[False if i in var_bound_constraint_ineq else True for i in range(0,A_ineq.shape[0])]]
            ...
            z_map_constr_eq = z_map_constr_eq[:,[False if i in var_bound_constraint_eq else True for i in range(0,A_eq.shape[0])]]
