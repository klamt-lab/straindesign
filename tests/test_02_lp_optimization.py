...
def test_yield_opt(curr_solver,model_weak_coupling):
    ...
    num = 'r_P'
    den = 'r_S'
    sol = sd.yopt(model_weak_coupling,
                            obj_num=num, obj_den=den,
                            solver=curr_solver, constraints=constr)
    assert(round(sol.objective_value,9) == 0.6)
    sol = sd.yopt(model_weak_coupling,obj_sense='min',
                            obj_num=num, obj_den=den,
                            solver=curr_solver, constraints=constr)
...
def test_yield_opt_unbounded(curr_solver,model_small_example):
    ...
    num = 'R4'
    den = 'R2'
    sol = sd.yopt(model_small_example,
                    obj_num=num, obj_den=den,
                    solver=curr_solver)
    assert(sol.status == sd.UNBOUNDED)
...
def load_solvers(model_small_example):
    """Test solver choice."""
    ...
    model_small_example.solver = 'scip'
    solver3 = sd.select_solver(None, model_small_example)
    assert(solver3 == SCIP)
