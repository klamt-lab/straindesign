...
def test_mcs(curr_solver, model_small_example, comp_approach, bigM,
             compression):
    modules = [
        sd.SDModule(model_small_example,
                    SUPPRESS,
                    constraints=["R3 - 0.5 R1 <= 0.0", "R2 <= 0", "R1 >= 0.1"])
    ]
    modules += [
        sd.SDModule(model_small_example,
                    SUPPRESS,
                    constraints=[
                        "1.0 R3 - 0.5 R1 - 0.5 R2 <= 0.0 ", "1.0 R2 >= 0.0 ",
                        "1.0 R1 >= 0.1 "
                    ])
    ]
    modules += [
...
def test_mcs_opt(curr_solver, model_weak_coupling, comp_approach, bigM,
    ...
    modules += [
        sd.SDModule(model_weak_coupling, PROTECT, constraints=["r_BM >= 0.2"])
    ]
    kocost = {
        'r1': 1,
        'r2': 1,
...
def test_mcs_gpr(model_gpr, comp_approach):
    ...
    modules = [
        sd.SDModule(model_gpr, SUPPRESS, constraints=["1.0 rd_ex >= 1.0 "])
    ]
    modules += [
        sd.SDModule(model_gpr,
                    PROTECT,
                    constraints=[[{
                        'r_bm': 1.0
                    }, '>=', 1.0]])
...
def test_optknock(curr_solver, model_weak_coupling, comp_approach_best_populate,
                  bigM, compression):
...
def test_robustknock(curr_solver, model_weak_coupling,
                     comp_approach_best_populate, bigM, compression):
    ...
    for s in solution.get_reaction_sd():
        ...
        sol_min_P = sd.fba(m,
                           ...
                           obj='r_P',
                           constraints="r_BM >= " +
                           str(sol_max_BM.objective_value))
        assert (sol_min_P.objective_value > 0)


def test_optcouple(curr_solver, model_weak_coupling,
                   comp_approach_best_populate, bigM, compression):
    ...
    for s in solution.get_reaction_sd():
        ...
        sol_max_BM_no_P = sd.fba(model_weak_coupling,
                                 obj='r_BM',
                                 constraints=constraints_no_P)
        max_BM_no_P = sol_max_BM_no_P.objective_value
        sol_max_BM = sd.fba(model_weak_coupling,
                            obj='r_BM',
        ...
        sol_min_P = sd.fba(model_weak_coupling,
                           obj_sense='minimize',
                           obj='r_P',
                           constraints=constraints_max_BM)
        assert (sol_min_P.objective_value > 0)
    pass
