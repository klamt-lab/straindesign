...
def _coupled_chain():
    ...
    vin = Reaction("vin")
    vin.add_metabolites({mets["A"]: 1})
    vin.bounds = (0, 10)
    r1 = Reaction("r1")
    r1.add_metabolites({mets["A"]: -1, mets["B"]: 1})
    r1.bounds = (0, 1000)
    r2 = Reaction("r2")
    r2.add_metabolites({mets["B"]: -1, mets["C"]: 1})
    r2.bounds = (0, 1000)
    r3 = Reaction("r3")
    r3.add_metabolites({mets["C"]: -1, mets["D"]: 1})
    r3.bounds = (0, 1000)
    vout = Reaction("vout")
    vout.add_metabolites({mets["D"]: -1})
    vout.bounds = (0, 1000)
    m.add_reactions([vin, r1, r2, r3, vout])
    m.objective = "vout"
    return m
...
def test_coupled_exemption_keeps_reaction_and_merges_rest():
    ...
    m_full = _coupled_chain()
    compress_model(m_full)
...
def _gene_model():
    ...
    vS = Reaction("vS")
    vS.add_metabolites({A: 1})
    vS.bounds = (0, 10)
    r1 = Reaction("r1")
    r1.add_metabolites({A: -1, B: 2})
    r1.bounds = (0, 1000)
    ...
    vC = Reaction("vC")
    vC.add_metabolites({C: -1})
    vC.bounds = (0, 1000)
    m.add_reactions([vS, r1, r2, vC])
    m.objective = "vC"
    return m
...
def _max_C_under_g1_le_1(protect):
    m = _gene_model()
    cmap = compress_model(m, no_coupled_compress_reacs=({"r1", "r2"} if protect else set()), propagate_gpr=True)
...
def test_gene_regulatory_multiplicity_preserved_under_compression():
    ...
    mu = _gene_model()
    extend_model_gpr(mu, use_names=False)
    mu.reactions.g1.upper_bound = 1.0
    ref = _fba(mu, {"vC": 1})
