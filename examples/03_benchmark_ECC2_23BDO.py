...
network = cobra.io.read_sbml_model(
    os.path.dirname(os.path.abspath(__file__)) + "/ECC2_23BDO.sbml")

# specify modules
modules = [
    straindesign.SDModule(
        network,
        PROTECT,
        constraints=["BIOMASS_Ec_iJO1366_core_53p95M >= 0.05"])
]
modules += [
    straindesign.SDModule(network,
                          SUPPRESS,
                          constraints=["EX_23bdo_e + 0.3 EX_glc__D_e <= 0"])
]
