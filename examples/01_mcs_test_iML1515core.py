...
network = cobra.io.read_sbml_model(
    os.path.dirname(os.path.abspath(__file__)) + "/iML1515core.sbml")
...
# e reactions
external_mets = [
    i for i, cpts in zip(network.metabolites,
...
obsolete_reacs = [
    reac for reac, b_rempty in zip(network.reactions, np.any(S, 0))
    if not b_rempty
]
...
modules = straindesign.SDModule(
    network,
    PROTECT,
    constraints=["2 BIOMASS_Ec_iML1515_core_75p37M >= 0.1", "EX_etoh_e >= 1"],
