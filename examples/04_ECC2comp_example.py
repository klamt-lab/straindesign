...
import cobra
import optlang
import straindesign.mcs_computation as mcs_computation
import numpy
import scipy
...
ecc2_mue_target = [
    mcs_computation.relations2leq_matrix(
        ...
        reac_id) for t in ecc2_mue_target
]
...
ecc2_mue_target_constraints = mcs_computation.get_leq_constraints(
    ecc2, ecc2_mue_target)
for c in ecc2_mue_target_constraints[
        0]:  # print constraints that make up the first target
    print(c)
#%%
ecc2_mcs, _ = mcs_computation.compute_mcs(ecc2,
                                          ecc2_mue_target,
                                          cuts=cuts,
                                          enum_method=3,
                                          max_mcs_size=3,
...
print(
    all(
        mcs_computation.check_mcs(ecc2, ecc2_mue_target[0], ecc2_mcs,
                                  optlang.interface.INFEASIBLE)))

# %% same calculation without network compression
ecc2_mcsF, _ = mcs_computation.compute_mcs(ecc2,
                                           ecc2_mue_target,
                                           cuts=cuts,
                                           enum_method=3,
                                           max_mcs_size=3,
...
# %%
import pickle
with open("ecc2_straindesign.pkl", "rb") as f:
    ref = pickle.load(f)
set(ecc2_mcs) - ref
...
# with ecc2 as model:
model = ecc2.copy(
)  # copy model because switching solver in context sometimes gives an error (?!?)
...
reduced = cobra.util.array.create_stoichiometric_matrix(ecc2c,
                                                        array_type='dok',
                                                        dtype=numpy.object)
